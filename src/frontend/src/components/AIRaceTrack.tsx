...
interface AIRacePhase {
  status: "pre" | "racing" | "finished";
  lap: number;
...
export default function AIRaceTrack({ onBack }: { onBack?: () => void }) {
  const [phase, setPhase] = useState<AIRacePhase>({
    status: "pre",
    lap: 1,
  ...
  const [playerCarState, setPlayerCarState] = useState<CarState>(() =>
    getInitialCarState(0),
  );
  const [aiCarState, setAiCarState] = useState<CarState>(() =>
    getInitialCarState(0.025),
  );
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      {/* Pre-race */}
      {phase.status === "pre" && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 via-orange-900 to-gray-900">
          ...
          <button
            type="button"
            onClick={startRace}
            className="px-10 py-4 bg-orange-600 hover:bg-orange-500 text-white text-2xl font-bold rounded-lg transition-colors"
          >
            START RACE
          </button>
          {onBack && (
        ...
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 via-orange-900 to-gray-900">
          ...
          {phase.finishTime !== null && (
            <p className="text-2xl text-yellow-400 mb-2">
              Time: {phase.finishTime.toFixed(2)}s
            </p>
          )}
          ...
          <button
            type="button"
            onClick={resetRace}
            className="px-10 py-4 bg-orange-600 hover:bg-orange-500 text-white text-2xl font-bold rounded-lg transition-colors mb-4"
          >
            RACE AGAIN
          </button>
          {onBack && (
